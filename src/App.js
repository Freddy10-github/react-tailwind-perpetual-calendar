...
function App() {
  ...
  useEffect(()=>{
    ...
    while(current.getMonth()>=date.getMonth()){
      ...
      else if(current.getDate()===date.getDate() && current.getMonth()===date.getMonth())
        str += `<li class="w-8 center text-center border">${date.getDate()}</li>`
      else
        str += `<li class="w-8 center text-center">${date.getDate()}</li>`
  ...
  useEffect(()=>{
    if(isReRender.current>0)
      return;
    const yearsSelect = document.querySelector('#years')
    for(let i = new Date().getFullYear()-20 ; i<new Date().getFullYear()+20 ; i+=1){
      if(i === new Date().getFullYear())
        yearsSelect.add(new Option(i,i,0,1))
      else
        yearsSelect.add(new Option(i,i))
    }
    ...
    for(let i = 0 ; i<12 ; i+=1){
      if(i === new Date().getMonth())
        ...
        daysSelect.add(new Option(i+1,i))
    }
  ...
  return (
    <div className="App">
      <h1 className="text-red-700">
        <select name="years" id="years" onChange={(e)=>{
        ...
        <select name="months" id="months"  onChange={(e)=>{
      ...
      <div>
        ...
        <ul className="flex list-none w-56 justify-start flex-wrap" id="days">
